...
from .image_handler import ImageHandlerRoborock
from .map_data import *
from .map_parser_const import *
from .types import Colors, Drawables, Sizes, Texts
...
class MapDataParserRoborock:
    ...
    def draw_elements(
        ...
        for drawable in drawables:
            if DRAWABLE_CHARGER == drawable and map_data.charger is not None:
                ImageHandlerRoborock.draw_charger(
                    map_data.image, map_data.charger, sizes, colors
                )
            if (
                DRAWABLE_VACUUM_POSITION == drawable
                and map_data.vacuum_position is not None
            ):
                ImageHandlerRoborock.draw_vacuum_position(
                    map_data.image, map_data.vacuum_position, sizes, colors
                )
            if DRAWABLE_OBSTACLES == drawable and map_data.obstacles is not None:
                ImageHandlerRoborock.draw_obstacles(
                    map_data.image, map_data.obstacles, sizes, colors
                )
            if (
                DRAWABLE_IGNORED_OBSTACLES == drawable
                and map_data.ignored_obstacles is not None
            ):
                ImageHandlerRoborock.draw_ignored_obstacles(
                    map_data.image, map_data.ignored_obstacles, sizes, colors
                )
            if (
                DRAWABLE_OBSTACLES_WITH_PHOTO == drawable
                and map_data.obstacles_with_photo is not None
            ):
                ImageHandlerRoborock.draw_obstacles_with_photo(
                    map_data.image, map_data.obstacles_with_photo, sizes, colors
                )
            if (
                DRAWABLE_IGNORED_OBSTACLES_WITH_PHOTO == drawable
                and map_data.ignored_obstacles_with_photo is not None
            ):
                ImageHandlerRoborock.draw_ignored_obstacles_with_photo(
                    map_data.image, map_data.ignored_obstacles_with_photo, sizes, colors
                )
            if DRAWABLE_MOP_PATH == drawable and map_data.mop_path is not None:
                ImageHandlerRoborock.draw_mop_path(
                    map_data.image, map_data.mop_path, sizes, colors, scale
                )
            if DRAWABLE_PATH == drawable and map_data.path is not None:
                ImageHandlerRoborock.draw_path(
                    map_data.image, map_data.path, sizes, colors, scale
                )
            if DRAWABLE_GOTO_PATH == drawable and map_data.goto_path is not None:
                ImageHandlerRoborock.draw_goto_path(
                    map_data.image, map_data.goto_path, sizes, colors, scale
                )
            if (
                DRAWABLE_PREDICTED_PATH == drawable
                and map_data.predicted_path is not None
            ):
                ImageHandlerRoborock.draw_predicted_path(
                    map_data.image, map_data.predicted_path, sizes, colors, scale
                )
            if (
                DRAWABLE_NO_CARPET_AREAS == drawable
                and map_data.no_carpet_areas is not None
            ):
                ImageHandlerRoborock.draw_no_carpet_areas(
                    map_data.image, map_data.no_carpet_areas, colors
                )
            if DRAWABLE_NO_GO_AREAS == drawable and map_data.no_go_areas is not None:
                ImageHandlerRoborock.draw_no_go_areas(
                    map_data.image, map_data.no_go_areas, colors
                )
            if (
                DRAWABLE_NO_MOPPING_AREAS == drawable
                and map_data.no_mopping_areas is not None
            ):
                ImageHandlerRoborock.draw_no_mopping_areas(
            ...
            if (
                DRAWABLE_CLEANED_AREA == drawable
                and DRAWABLE_CLEANED_AREA in map_data.image.additional_layers
            ):
                ImageHandlerRoborock.draw_layer(map_data.image, drawable)
            if DRAWABLE_ROOM_NAMES == drawable and map_data.rooms is not None:
                ImageHandlerRoborock.draw_room_names(
                    map_data.image, map_data.rooms, colors
                )
    ...
    def parse(
        ...
        **kwargs,
        ...
        while block_start_position < len(raw):
            block_header_length = MapDataParserRoborock.get_int16(
                raw, block_start_position + 0x02
            )
            header = MapDataParserRoborock.get_bytes(
            ...
            data = MapDataParserRoborock.get_bytes(
                raw, block_data_start, block_data_length
            )

            if block_type == MapDataParserRoborock.CHARGER:
                map_data.charger = MapDataParserRoborock.parse_object_position(
                ...
                map_data.vacuum_position = MapDataParserRoborock.parse_object_position(
                    block_data_length, data
                )
            elif block_type == MapDataParserRoborock.PATH:
                map_data.path = MapDataParserRoborock.parse_path(
                    block_start_position, header, raw
                )
            elif block_type == MapDataParserRoborock.GOTO_PATH:
                map_data.goto_path = MapDataParserRoborock.parse_path(
                    block_start_position, header, raw
                )
            elif block_type == MapDataParserRoborock.GOTO_PREDICTED_PATH:
                map_data.predicted_path = MapDataParserRoborock.parse_path(
                    block_start_position, header, raw
                )
                ...
                block_type == MapDataParserRoborock.NO_GO_AREAS
                and image_config[CONF_INCLUDE_NOGO]
            ):
                map_data.no_go_areas = MapDataParserRoborock.parse_area(header, data)
            elif block_type == MapDataParserRoborock.NO_MOPPING_AREAS:
                map_data.no_mopping_areas = MapDataParserRoborock.parse_area(
                    header, data
                )
            elif block_type == MapDataParserRoborock.OBSTACLES:
                map_data.obstacles = MapDataParserRoborock.parse_obstacles(data, header)
            elif (
                block_type == MapDataParserRoborock.IGNORED_OBSTACLES
                and image_config[CONF_INCLUDE_IGNORED_OBSTACLES]
            ):
                map_data.ignored_obstacles = MapDataParserRoborock.parse_obstacles(
                    data, header
                )
            elif block_type == MapDataParserRoborock.OBSTACLES_WITH_PHOTO:
                map_data.obstacles_with_photo = MapDataParserRoborock.parse_obstacles(
                    data, header
                )
            elif block_type == MapDataParserRoborock.IGNORED_OBSTACLES_WITH_PHOTO:
                map_data.ignored_obstacles_with_photo = (
                    MapDataParserRoborock.parse_obstacles(data, header)
                )
                ...
                points_mask = MapDataParserRoborock.get_bytes(
                    raw, block_data_start, block_data_length
                )
                # only the map_data.path points where points_mask == 1 are in mop_path
                map_data.mop_path = MapDataParserRoborock.parse_mop_path(
                    map_data.path, points_mask
                )
                ...
                map_data.carpet_map = MapDataParserRoborock.parse_carpet_map(
                    data, image_config
                )
            elif block_type == MapDataParserRoborock.NO_CARPET_AREAS:
                map_data.no_carpet_areas = MapDataParserRoborock.parse_area(
                    header, data
                )
            else:
                _LOGGER.debug(
                    "UNKNOWN BLOCK TYPE: %s, header length %s, data length %s",
                    block_type,
                    block_header_length,
                    block_data_length,
                )
            block_start_position = (
                block_start_position
                + block_data_length
                + MapDataParserRoborock.get_int8(header, 2)
            )

        if img_data:
            image, rooms = MapDataParserRoborock.parse_image(
                img_data_length,
                img_header_length,
                img_data,
                img_header,
    ...
    @staticmethod
    def get_current_vacuum_room(
        block_start_position: int, raw: bytes, vacuum_position: Point
    ) -> int:
        block_header_length = MapDataParserRoborock.get_int16(
            raw, block_start_position + 0x02
        )
        ...
        room = ImageHandlerRoborock.get_room_at_pixel(
            data, image_width, round(p.x - image_left), round(p.y - image_top)
        )
        return room
    ...
    def parse_image(
        ...
        image, rooms_raw = ImageHandlerRoborock.parse(
            data, image_width, image_height, carpet_map, colors, image_config
        )
        rooms = {}
        for number, room in rooms_raw.items():
            rooms[number] = Room(
                number,
                MapDataParserRoborock.image_to_map(room[0] + image_left),
        ...
        return (
            ImageData(
                ...
                image_config,
                image,
                MapDataParserRoborock.map_to_image,
            ),
            rooms,
        )
    ...
    def parse_obstacles(data: bytes, header: bytes) -> List[Obstacle]:
        ...
        for obstacle_start in range(0, obstacle_pairs * obstacle_size, obstacle_size):
            ...
            details = {}
            if obstacle_size >= 6:
                details[ATTR_TYPE] = MapDataParserRoborock.get_int16(
                    data, obstacle_start + 4
                )
                if details[ATTR_TYPE] in MapDataParserRoborock.KNOWN_OBSTACLE_TYPES:
                    details[
                        ATTR_DESCRIPTION
                    ] = MapDataParserRoborock.KNOWN_OBSTACLE_TYPES[details[ATTR_TYPE]]
                if obstacle_size >= 10:
                    ...
                    if obstacle_size == 28 and (data[obstacle_start + 12] & 0xFF) > 0:
                        txt = MapDataParserRoborock.get_bytes(
                            data, obstacle_start + 12, 16
                        )
