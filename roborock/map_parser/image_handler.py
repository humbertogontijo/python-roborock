...
class ImageHandlerRoborock:
    ...
    @staticmethod
    def draw_path(
        image: ImageData, path: Path, sizes: Sizes, colors: Colors, scale: float
    ):
        ImageHandlerRoborock.__draw_path__(
            image,
            path,
            sizes[CONF_SIZE_PATH_WIDTH],
            ImageHandlerRoborock.__get_color__(COLOR_PATH, colors),
            scale,
        )
    ...
    def draw_goto_path(
        ...
        ImageHandlerRoborock.__draw_path__(
            ...
            path,
            sizes[CONF_SIZE_PATH_WIDTH],
            ImageHandlerRoborock.__get_color__(COLOR_GOTO_PATH, colors),
            scale,
        )
    ...
    def draw_predicted_path(
        ...
        ImageHandlerRoborock.__draw_path__(
            ...
            path,
            sizes[CONF_SIZE_PATH_WIDTH],
            ImageHandlerRoborock.__get_color__(COLOR_PREDICTED_PATH, colors),
            scale,
        )
    ...
    def draw_mop_path(
        ...
        ImageHandlerRoborock.__draw_path__(
            image,
            path,
            sizes[CONF_SIZE_MOP_PATH_WIDTH],
            ImageHandlerRoborock.__get_color__(COLOR_MOP_PATH, colors),
            scale,
        )

    @staticmethod
    def draw_no_carpet_areas(image: ImageData, areas: List[Area], colors: Colors):
        ImageHandlerRoborock.__draw_areas__(
            image,
            areas,
    ...
    def draw_walls(image: ImageData, walls: List[Wall], colors: Colors):
        ...
        for wall in walls:
            draw.line(
                ...
                ImageHandlerRoborock.__get_color__(COLOR_VIRTUAL_WALLS, colors),
                width=2,
            )
    ...
    @staticmethod
    def draw_ignored_obstacles(
        image: ImageData, obstacles: List[Obstacle], sizes: Sizes, colors: Colors
    ):
    ...
    @staticmethod
    def draw_ignored_obstacles_with_photo(
        image: ImageData, obstacles: List[Obstacle], sizes: Sizes, colors: Colors
    ):
        color = ImageHandlerRoborock.__get_color__(
            COLOR_IGNORED_OBSTACLE_WITH_PHOTO, colors
        )
    ...
    def draw_all_obstacles(
        ...
        outline_color = ImageHandlerRoborock.__get_color__(
            COLOR_OBSTACLE_OUTLINE, colors
        )
        for obstacle in obstacles:
            ImageHandlerRoborock.__draw_circle__(
                image, obstacle, radius, outline_color, color
            )

    @staticmethod
    def draw_vacuum_position(
        image: ImageData, vacuum_position: Point, sizes: Sizes, colors: Colors
    ):
        ...
        ImageHandlerRoborock.__draw_vacuum__(
            image, vacuum_position, radius, outline, color
        )
    ...
    def draw_room_names(image: ImageData, rooms: Dict[int, Room], colors: Colors):
        ...
        for room in rooms.values():
            ...
            if p is not None:
                ...
                ImageHandlerRoborock.__draw_text__(
                    image, room.name, point.x, point.y, color
                )
    ...
    def draw_texts(image: ImageData, texts: Texts):
        for text_config in texts:
            ...
            ImageHandlerRoborock.__draw_text__(
                image,
                text_config[CONF_TEXT],
                x,
                y,
                text_config[CONF_COLOR],
                text_config[CONF_FONT],
                text_config[CONF_FONT_SIZE],
            )
    ...
    def __draw_vacuum__(image: ImageData, vacuum_pos, r, outline, fill):
        ...
        ImageHandlerRoborock.__draw_on_new_layer__(
            image,
            draw_func,
            1,
    ...
    def __draw_areas__(
        ...
        for area in areas:
            ...
            def draw_func(draw: ImageDraw):
                draw.polygon(
                    current_area.to_img(image.dimensions).as_list(), fill, outline
                )

            ImageHandlerRoborock.__draw_on_new_layer__(
                image, draw_func, 1, use_transparency
            )

    @staticmethod
    def __draw_path__(
        image: ImageData, path: Path, path_width: int, color: Color, scale: float
    ):
        if len(path.path) < 1:
            return
            ...
            for current_path in path.path:
                if len(current_path) > 1:
                    ...
                    for point in current_path[1:]:
                        ...
                        ey = e.y * scale
                        draw.line(
                            [sx, sy, ex, ey], width=int(scale * path_width), fill=color
                        )
                        if path_width > 4:
        ...
        ImageHandlerRoborock.__draw_on_new_layer__(
            image, draw_func, scale, ImageHandlerRoborock.__use_transparency__(color)
        )
    ...
    def __draw_text__(
        ...
        ImageHandlerRoborock.__draw_on_new_layer__(
            image, draw_func, 1, ImageHandlerRoborock.__use_transparency__(color)
        )
        ...
        for img_y in range(trimmed_height):
            for img_x in range(trimmed_width):
                ...
                if pixel_type == ImageHandlerRoborock.MAP_OUTSIDE:
                    pixels[x, y] = ImageHandlerRoborock.__get_color__(
                ...
                if idx in carpet_map and (x + y) % 2:
                    ...
                    carpet_color = ImageHandlerRoborock.__get_color__(
                        COLOR_CARPETS, colors
                    )
                    ...
                    if not pixels[x, y] or len(carpet_color) != 4:
                        ...
                        pixels[x, y] = (
                            combine_color_component(
                                pixels[x, y][0], carpet_color[0], carpet_color[3]
                            ),
                            combine_color_component(
                                pixels[x, y][1], carpet_color[1], carpet_color[3]
                            ),
                            combine_color_component(
                                pixels[x, y][2], carpet_color[2], carpet_color[3]
                            ),
                        )
    ...
    def get_room_at_pixel(raw_data: bytes, width: int, x: int, y: int) -> int:
        ...
        if pixel_type not in [
            ImageHandlerRoborock.MAP_INSIDE,
            ImageHandlerRoborock.MAP_SCAN,
        ]:
